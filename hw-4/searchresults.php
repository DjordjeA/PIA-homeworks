...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $search = $_POST['genre']; 
  $results = array("Akcija", "Avantura", "Biografski", "Komedija", "Kriminalisticki", "Dokumentarni", "Drama", "Porodicni", "Fantazija", "Istorija", "Horor", "Misterija", "Romanticni", "Sci-Fi", "Triler", "Ratni", "Vestern");
  $match = $results[($search-1)]; 
...
<div class="div-padding">
  <span class="bolda">Filmovi: "<?php echo $match; ?>"</span>
</div><br>

<?php


    $sqlp2 = "SELECT mid FROM link  WHERE gid = $search";
    $resultp2 = $conn->query($sqlp2);
    if ($resultp2->num_rows > 0){
      while ($rowp2 = $resultp2->fetch_assoc()){

        $sql = "SELECT id, name, year, trailer, addedBy FROM movie WHERE id = $rowp2[mid]"; 
        ...
        if ($result->num_rows > 0) {
             while($row = $result->fetch_assoc()) { 
                 ...
                 echo "<div class='padding'><span class='movie-info'>Genre: "; 
                 ...
                 echo "</div><div class='padding movie-info'>Added by: " . $addedBy . "</div>"; 
                 ...
                 } else { echo "Nema trejlera za ovaj film.<br><br>"; }
                 echo "</span></div><br>";
             }
        }
      }
    } else { echo "<div class='noButton'><br>Nije pronadjen nijedan film.<br><br></div>"; } 
